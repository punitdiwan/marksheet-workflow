...
async function replaceImageInOdt(templatePath, student, schoolDetails, tempDir) {
    ...
    // 2. Parse Content AND Manifest XML
    try {
        const contentXmlStr = await fs.readFile(contentXmlPath, 'utf-8');
        const manifestXmlStr = await fs.readFile(manifestPath, 'utf-8');

        // Use strict settings to preserve structure
        const xmlOptions = { explicitArray: true, mergeAttrs: false };
        const contentXml = await parseXml(contentXmlStr, xmlOptions);
        const manifestXml = await parseXml(manifestXmlStr, xmlOptions);

        // Helper to find frames in content.xml
        const allFrames = [];
        function findFrames(node) {
        ...
        for (const replacement of imageReplacements) {
            ...
            if (targetFrame && targetFrame['draw:image'] && targetFrame['draw:image'][0]) {
                ...
                if (imageBuffer) {
                    // A. Create Unique Filename
                    const safeName = frameName.replace(/[^a-zA-Z0-9]/g, '');
                    const uniqueFilename = `${safeName}_${Date.now()}_${Math.floor(Math.random() * 1000)}.png`;
                    const newImagePath = path.join(picturesDir, uniqueFilename);

                    // B. Write File
                    await fs.writeFile(newImagePath, imageBuffer);

                    // C. Update content.xml Reference
                    targetFrame['draw:image'][0].$['xlink:href'] = `Pictures/${uniqueFilename}`;

                    // D. Update manifest.xml (CRITICAL STEP)
                    // We must add the new file to the manifest so LibreOffice knows it exists.
                    if (manifestXml['manifest:manifest'] && manifestXml['manifest:manifest'][0] &&
                        manifestXml['manifest:manifest'][0]['manifest:file-entry']) {

                        manifestXml['manifest:manifest'][0]['manifest:file-entry'].push({
                            $: {
                                'manifest:full-path': `Pictures/${uniqueFilename}`,
                    ...
                    xmlModified = true;
                    console.log(`✅ Linked ${frameName} to ${uniqueFilename} and updated manifest.`);
                }
            }
        }
        ...
        if (xmlModified) {
            ...
            await fs.writeFile(contentXmlPath, builder.buildObject(contentXml));
            await fs.writeFile(manifestPath, builder.buildObject(manifestXml));
        }

    } catch (e) {
        console.error("❌ Critical error modifying XML:", e);
    }
    ...
    // A. Add mimetype FIRST (No Compression)
    ...
    try {
        ...
        console.warn('⚠️ mimetype file missing.');
    }
    ...
    const walkDir = async (dir, relativeRoot) => {
        ...
        for (const file of files) {
            if (file === 'mimetype' && relativeRoot === '') continue; // Skip mimetype
